...
async function downloadWithCurl(url, filepath) {
  try {
    execSync(`curl -s -L -o "${filepath}" "${url}"`, { stdio: 'pipe' });
    ...
    // Intentar con wget
    try {
      execSync(`wget -q -O "${filepath}" "${url}"`, { stdio: 'pipe' });
      if (fs.existsSync(filepath) && fs.statSync(filepath).size > 0) {
        return true;
      }
    } catch (e2) {
...
async function main() {
  console.log('📥 Descargando imágenes con curl/wget...\n');
  ...
  for (const url of urls) {
    ...
    const success = await downloadWithCurl(url, tempPath);

    if (success) {
      console.log(`  ✅ Descargado (${formatBytes(getFileSize(tempPath))})`);
      downloaded++;
    } else {
      console.log(`  ❌ Error`);
    }
  ...
  if (downloaded === 0) {
    console.log('⚠️  No se pudo descargar ninguna imagen debido a restricciones de red.');
    console.log('Por favor ejecuta este script en tu entorno local donde tengas acceso a Internet.\n');
